...
static locl_t lo;
weit_t w[ A + P ][ E ];
weit_t *wp[ A + P ];					//pointer array for sorting
...
void dp_init( data_t *g, weit_t *w, locl_t *lp );
void ecmpwarshallfroyd( locl_t *lp );
inline unsigned int bit_count(flag_t n);
void alloc_flow(locl_t *lp, double flow, unsigned int rate, int i, int j);
void buildnexthoplist( locl_t *lp, int i, int j );
void c_flow( data_t *g, weit_t *w, locl_t *lp );
void mating( weit_t *c, const weit_t *el, const weit_t *nel );
void geneSorting( data_t **gp, weit_t **wp );
void geneEvolution( data_t **gp, weit_t **wp, weit_t *c );
...
void c_flow( data_t *g, weit_t *w, locl_t *lp ) {
  ...
  dp_init( g, w, lp );
  ecmpwarshallfroyd( lp );
	
  for( i = 0; i < N; i++ ) {
    for( j = 0; j < N; j++ ) {
      buildnexthoplist( lp, i, j );
    }
  }
  ...
  for ( i = 0; i < N; i++) {
    for ( j = 0; j < N; j++) {
      if( ( i != j ) && dem_p[i][j] ) {
        alloc_flow(lp, dem_p[i][j], INITIAL_RATE, i, j);
      }
    }
  }
...
void geneEvolution( data_t **gp, weit_t **wp, weit_t *cb ) {
  int i,j;
  weit_t *elite, *nonelite, *c_p;
	
  for( i = A; i < A+B; i++ ) {				//elite => keep the same	,
    elite = wp[random() % A ];				//elite
    nonelite = wp[A + random() % P ];	//nonelite
    mating( &cb[ (i-A) * E ], elite, nonelite );	//mating => ( nonelite + elite )
  }
	
  for( i = A; i < A+B; i++ ) {	//update it back
    for( j = 0; j < E; j++ ) {
      wp[i][j] = cb[ (i-A) * E + j ];			//copy
    }
  }

  for( i = A+B; i < A+P; i++ ) {	  //replace it with random one;
    for( j = 0; j < E; j++ ) {
      wp[i][j] = ( random() % WMAX ) + 1;
      gp[i]->L = 100.0;
    }
  }
}
...
int main(){
  ...
  for( i = 0 ; i < P+A; i++ ) {		//init process
    g[i].L = 100.0;
    for ( j = 0; j < E; j++) {
			w[i][j] = ( random() % WMAX ) + 1;		//initialize
    }
    gp[i] = &(g[i]);
		wp[i] = w[i];
  }
	
#pragma omp parallel num_threads(MP) firstprivate(i)
  for( i = 0; i < GENMAX; i++ ){
#pragma omp for private(lo)
    for( j = A; j < A+P; j++) {
      c_flow( gp[j], wp[j], &(lo) );							//calculation flow
    }
#pragma omp single
		{
			geneSorting(gp, wp);			//gene sort
			geneEvolution(gp, wp, cb);		//gene evolution
